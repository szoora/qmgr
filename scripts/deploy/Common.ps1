...
function Get-RepoBuildVersion {
    ...
    $hash  = ''
    try { $hash = (& git -c safe.directory=* -C $RepoRoot rev-parse --short HEAD 2>$null).Trim() } catch { }
    # A package built from uncommitted work says so (2026-09-25): the hash alone names a commit the
    # binaries do not match, and the health endpoint then reports a version nobody can check out.
    $dirty = ''
    try {
        $status = (& git -c safe.directory=* -C $RepoRoot status --porcelain 2>$null)
        if ($status) { $dirty = '.dirty' }
    } catch { }
    if ($hash) { return "$base+$stamp.$hash$dirty" } else { return "$base+$stamp$dirty" }
}
...
function Write-BuildInfo {
    ...
    $hash   = ''
    try { $hash   = (& git -c safe.directory=* -C $RepoRoot rev-parse --short HEAD 2>$null).Trim() } catch { }
    try { $branch = (& git -c safe.directory=* -C $RepoRoot rev-parse --abbrev-ref HEAD 2>$null).Trim() } catch { }
    $buildInfo = @"
