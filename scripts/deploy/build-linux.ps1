...
# The sudoers entry: ONE command, no password, nothing else. NOPASSWD is required because the API
# process has no terminal — a prompt would hang the request rather than fail it.
$tenantSudoers = @"
# Q-Mgr — generated by scripts/deploy/build-linux.ps1 — build $BuildVersion
# Lets the API point a verified tenant domain at this server's certificate, and nothing else.
www-data ALL=(root) NOPASSWD: /usr/local/bin/qmgr-tenant-domain
"@
Write-LinuxText -Path (Join-Path $genDir 'qmgr-tenant-domain.sudoers') -Content $tenantSudoers
Write-Success 'tenant custom-domain helper and sudoers entry generated'
...
# visudo -c on the single file, because a malformed sudoers drop-in locks sudo out of the box.
install -m 0440 -o root -g root "$PKG_DIR/config/qmgr-tenant-domain.sudoers" /etc/sudoers.d/qmgr-tenant-domain
if ! visudo -c -f /etc/sudoers.d/qmgr-tenant-domain >/dev/null; then
    echo -e "${RED}    sudoers drop-in was rejected — removing it${NC}"
    rm -f /etc/sudoers.d/qmgr-tenant-domain
fi
echo -e "${GREEN}    Tenant custom-domain helper installed${NC}"
